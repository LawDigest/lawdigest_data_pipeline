...
Usage 예시:
  python scripts/run_n8n_db_pipeline.py --step all --start-date 2025-12-01 --end-date 2025-12-07 --age 22
  python scripts/run_n8n_db_pipeline.py --step bills --start-date 2025-12-01 --end-date 2025-12-07
"""
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--step",
        choices=["all", "lawmakers", "bills", "timeline", "result", "vote", "stats"],
        default="all",
...
def _run_step(wfm: Any, step: str, args: argparse.Namespace) -> None:
    if step == "lawmakers":
        print("[INFO] step=lawmakers 시작")
        result = wfm.update_lawmakers_data(run_stats=not args.post_stats)
        count = len(result) if result is not None else 0
        print(f"[INFO] step=lawmakers 완료 ({count}건)")
        return

    if step == "bills":
        print("[INFO] step=bills 시작")
        ...
        count = len(result) if result is not None else 0
        print(f"[INFO] step=bills 완료 ({count}건)")
        return

    if step == "timeline":
        print("[INFO] step=timeline 시작")
        ...
        count = len(result) if result is not None else 0
        print(f"[INFO] step=timeline 완료 ({count}건)")
        return

    if step == "result":
        print("[INFO] step=result 시작")
        ...
        count = len(result) if result is not None else 0
        print(f"[INFO] step=result 완료 ({count}건)")
        return

    if step == "vote":
        print("[INFO] step=vote 시작")
        ...
        print(f"[INFO] step=vote 완료 (본회의 {vote_count}건, 정당별 {vote_party_count}건)")
        return

    if step == "stats":
        print("[INFO] step=stats 시작")
        wfm.update_statistics()
        print("[INFO] step=stats 완료")
...
def main() -> None:
    ...
    if args.step == "all":
        ...
        if args.post_stats:
            print("[INFO] --post-stats 활성화: all 모드의 마지막 stats 단계는 생략됩니다.")
    ...
    try:
        ...
        print(f"[ERROR] step 실행 중 오류가 발생했습니다: {type(e).__name__}: {e}")
        print(traceback.format_exc())
        raise

    print(f"[INFO] 완료: step={args.step}")
